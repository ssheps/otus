...
(
    ...
    constraint id
        unique (id),
    constraint order_statuses_name_uindex
        unique (name)
);
...
(
    ...
    added_to_order int(1)         default 0    not null comment '--т.к. в MySQL нет как такового типа boolean, приходится пользоваться таким способом',
    constraint id
        unique (id),
